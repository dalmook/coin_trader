...
def run_backtest(markets: list[str], count: int, unit: int, out_dir: str = "results"):
    cmd = ["python", "src/upbit_backtest.py", "--markets", *markets, "--count", str(count), "--unit", str(unit), "--out", out_dir]
...
def pick_best_combo(summary="results/all_summary.csv", quote="KRW", min_trades=3, max_mdd_pct=20.0, min_sharpe=0.2):
    try:
        rows = parse_summary(summary)
    except FileNotFoundError:
        return f"{quote}-BTC", "breakout_k05_trend", "fallback(no summary)"
    ...
    pool = filtered if filtered else [r for r in rows if r["market"].startswith(f"{quote}-")]
    if not pool:
        return f"{quote}-BTC", "breakout_k05_trend", "fallback(no rows)"
...
class Config:
    ...
    reselect_backtest_count: int = int(os.getenv("RESELECT_BACKTEST_COUNT", "600"))
...
def run():
    ...
    while True:
        ...
        if (cfg.market.upper()=="AUTO" or cfg.strategy.upper()=="AUTO") and (time.time() - last_reselect_ts >= cfg.reselect_interval_min * 60):
            old_market, old_strategy = market, strategy
            try:
                run_backtest(markets_for_reselect, cfg.reselect_backtest_count, cfg.unit, "results")
                market, strategy, reason = pick_best_combo(min_trades=cfg.select_min_trades, max_mdd_pct=cfg.select_max_mdd_pct, min_sharpe=cfg.select_min_sharpe)
                base, coin = market.split("-")
                ...
                if market != old_market or strategy != old_strategy:
                    msg = (f"🔄 Reselect Changed\n"
                           f"- {old_market}/{old_strategy} -> {market}/{strategy}\n"
                           f"- reason: {reason}")
                    try:
                        send_telegram(cfg.telegram_bot_token, cfg.telegram_chat_id, msg)
